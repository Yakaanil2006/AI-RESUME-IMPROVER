...
Format response starting exactly with:
MATCH_SCORE: <number>
...
with col_input:
    with st.container(border=True):
        ...
        # ✅ DateTime Input
        analysis_datetime = st.datetime_input(
            label="Select Analysis Date & Time",
            value=datetime.now(),
...
with col_result:
    if analyze_btn and resume_file:
            ...
            if text:
                ...
                with st.container(border=True):
                    ...
                    <div style="width:100%; background:#1e293b; border-radius:10px; height:10px;">
                        <div style="width:{score}%; background:linear-gradient(90deg,#22c55e,#4ade80); height:100%; border-radius:10px;"></div>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    st.markdown(
                        f"<p style='color:#94a3b8; text-align:center;'>Analysis Time: {analysis_datetime}</p>",
                        unsafe_allow_html=True
                    )

                    final_report = f"Analysis Time: {analysis_datetime}\n\n{clean_report}"
        ...
        <div style="text-align:center; padding:7rem 2rem; border:2px dashed rgba(255,255,255,0.1); border-radius:20px;">
            ...
            <p style="color:#64748b;">Upload your resume and provide a JD to see your match accuracy.</p>
        </div>
        """, unsafe_allow_html=True)

st.markdown(
    "<div style='text-align:center; padding:2rem; color:#475569; font-size:0.8rem;'>ResumePro AI • 2025</div>",
    unsafe_allow_html=True
)
